...
class Downloader {
    ...
    generatePost(post) {
        ...
        return async function () {
            ...
            const { title, slug, format, cover, created_at, updated_at, body } = post
            ...
            const fileName = `${filenamify(title)} ${formatDate(created_at)}.md`;
            const content = decoratePost(title, mdBody, `https://www.yuque.com/${client.namespace}/${slug}`)
            await gh.writeFile(`${repo_name}/${fileName}`, content)
